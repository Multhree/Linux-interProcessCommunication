...
{	
	struct msg data;
	int msgid1 = msgget((key_t)KEY, 0666|IPC_CREAT);
	if(msgid1 < 0)
	{
	...
	int msgid2= msgget((key_t)(KEY+1), 0666|IPC_CREAT);
	if(msgid2 < 0)
	{
	...
	while(1)
	{
		...
		if(op == 0)
		{
			...
			msgsnd(msgid2, &data,MESSAGEMAXSIZE,0);
		}	
		else if(op == 1)
		{			
			msgrcv(msgid1, &data,MESSAGEMAXSIZE,RECEIVETYPE,0);
			printf("msg: %s\n",data.message);
		}
		else if(op == 2)
		{
